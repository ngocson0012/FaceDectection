...
namespace Emgu.CV
{
   ...
   public class EigenObjectRecognizer
   {
      ...
      public float[] distf=null;
      public void FindMostSimilarObject1(Image<Gray, Byte> face, out int index, out float eigenDistancef, out String label)
      {
          distf = GetEigenDistances(face);
         // float[] disteL= GetEigenDistances(eyeL);
          //float[] disteR = GetEigenDistances(eyeR);
       //   float[] distn = GetEigenDistances(nose);
          //float[] distm = GetEigenDistances(mouth);
          int len = distf.Length;
          index = 0;
          eigenDistancef = distf[0];
         // MessageBox.Show(len.ToString());
          for (int i = 1; i < len; i++)
          {
              if (distf[i] < eigenDistancef)
              {
                  index = i;
                  eigenDistancef = distf[i];
                //  Console.WriteLine(distf[i]);
                 
              }
      //        MessageBox.Show(distf[i].ToString() + " : " + i.ToString());
           //  Console.WriteLine(distf[i].ToString());
          }
         // MessageBox.Show(distf.Length.ToString());
         // MessageBox.Show("AAAAAAAAAAA " + eigenDistancef.ToString());
          label = Labels[index];
      }
      /// <summary>
      /// Try to recognize the image and return its label
      /// </summary>
      /// <param name="face">The image to be recognized</param>
      /// <returns>
      ...
      public String Recognize(Image<Gray, Byte> face)
      {
         ...
         float eigenDistance;
         String label;
         FindMostSimilarObject1(face, out index, out eigenDistance, out label);
